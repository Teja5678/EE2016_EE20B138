...
def fileread():
    ...
    if(len(sys.argv)!=2):
        print("Please make sure arguments must be 2")
        exit()
...
def append(n1,n2):
    ...
    if(n1 not in node_names):
        node_names.append(n1)
        dummy = Node(n1,node_names.index(n1))
        node.append(dummy)
        
    if(n2 not in node_names):
        node_names.append(n2)
        dummy = Node(n2,node_names.index(n2))
        node.append(dummy)
    return node_names.index(n1),node_names.index(n2)

#break into components  and create an object for the component
def parse_line(line):
    tokens = line.split()
    l = len(tokens)
